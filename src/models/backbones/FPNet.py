...
class FPNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (hidden_channels, num_layers) in enumerate(zip(hidden_channels_list, num_layers_list)):
    ...
    def forward(self, info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        x_feat, edge_index, batch, graph_feat = info['x'], info['edge_index'], info['batch'], None
        ...
        for i in range(0, len(self.pyramid), 2):
            x_proj, graph_proj = self.pyramid[i](x_feat), self.pyramid[i](graph_feat) if graph_feat is not None else None
            x_feat, graph_feat = self.pyramid[i + 1](x_proj, edge_index, batch, graph_proj)
        ...
        # Output projection
        info['node_embed'], info['graph_embed'] = self.fusion(x_fused), self.fusion(graph_fused)
