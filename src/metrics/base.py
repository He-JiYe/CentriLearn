...
class BaseMetric(ABC):
    ...
    def update(self, value: float):
        ...
        if self.record == 'max':
            self.max_history = value if self.max_history is None else max(self.max_history, value)
        elif self.record == 'min':
            self.min_history = value if self.min_history is None else min(self.min_history, value)
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(name={self.name}, current={self.compute():.4f})"
