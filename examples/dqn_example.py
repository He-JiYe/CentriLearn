...
This example demonstrates how to train a DQN agent on the Network Dismantling task
using the configuration file configs/network_dismantling/dqn.yaml.
"""

import torch
import yaml

from centrilearn.utils import build_algorithm, build_environment, train_from_cfg


# Method 1: Training from config file (recommended)
def train_from_config_file():
    """Train DQN using the YAML config file."""
    print("=" * 60)
    print("Method 1: Training from config file")
    print("=" * 60)

    # Load configuration
    config_path = "configs/network_dismantling/dqn.yaml"
    ...
    save_path = "checkpoints/network_dismantling/dqn_trained.pth"
...
def train_with_custom_config():
    """Train DQN with custom configuration."""
    print("\n" + "=" * 60)
    ...
    config = {
        "algorithm": {
            ...
            "model": {
                ...
                "q_head_cfg": {"type": "QHead", "in_channels": 64},
            },
...
def train_step_by_step():
    """Train DQN step by step for more control."""
    print("\n" + "=" * 60)
    ...
    algo_cfg = {
        ...
        "replaybuffer_cfg": {"type": "ReplayBuffer", "capacity": 10000, "n_step": 5},
        "algo_cfg": {
            "gamma": 0.99,
            "epsilon_start": 1.0,
            "epsilon_end": 0.01,
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            # Select action
            action, epsilon = algorithm.select_action(state)

            # Execute action
            next_state, reward, done, info = env.step(action)

            # Collect experience
            algorithm.collect_experience(state, action, reward, next_state, done)

            # Update model
            if len(algorithm.replay_buffer) > batch_size:
...
# Method 4: Evaluation
def evaluate_trained_model(checkpoint_path="checkpoints/network_dismantling/dqn_trained.pth"):
...
def main():
    """Run all examples."""
    print("\n" + "=" * 60)
    print("CentriLearn DQN Training Examples")
    print("=" * 60)
